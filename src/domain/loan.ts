...
export function createLoan(params: LoanParams): Loan {
  const { id, deviceId, deviceName, loanStartDate, loanDueDate, user } = params;
  ...
  if (!(loanStartDate instanceof Date) || isNaN(loanStartDate.getTime())) {
    throw new LoanError(
      'loanStartDate',
      'Loan start date is required and must be a valid Date.'
    );
  }
  if (!(loanDueDate instanceof Date) || isNaN(loanDueDate.getTime())) {
    throw new LoanError(
      'loanDueDate',
      'Loan due date is required and must be a valid Date.'
    );
  }
  ...
  if (!user || typeof user !== 'string') {
    throw new LoanError('user', 'User is required and must be a string.');
  }
